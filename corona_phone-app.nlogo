turtles-own [
  ...
  my_risk
  my_move
  my_health_status ; healthy (0) / slightly ill (1) / severely ill (2) / dead (3)
  my_covid_type ; not contaminated (0) soft (1) or severe or critical (2)
  in_hospital ; no (0) / yes (1)
  wear_mask ; no (0) / yes (1)
  has_app ; no (0) / yes (1)
  my_app_status ; (0) not blocked / (1) blocked
  my_list_contact ; list of turtles met with the app
  time_since_blocked_by_app
]
...
to go
  ...
  save_list_app-turtles
  contaminate
  ask turtles [alert_list_app-turtles]
  ask turtles [do_you_go_to_hospital]
  ask turtles [do_you_die]
  ask turtles [get_older]
  ask turtles [do_you_deblock]
  update-display
...
to setup-turtles ;creation of the population group
  create-turtles 1000 [
    ...
    set my_covid_stat 0
    set my_group 1
    set my_risk 2 ; risk to die of virus in percent
    set my_move 3
    set my_health_status 0 ; in good shape
    set my_covid_type 0
    set in_hospital 0
    set my_list_contact (list 0)
  ]
  ask turtles [ setxy random-xcor random-ycor ]
  ask n-of 2 turtles
    [ get-sick ]
  ask n-of (who_has_the_app * 10) turtles [set has_app 1 set my_app_status 0]
end
...
to update-display
  ...
  if my_covid_stat = 1 [ set color red ]
  if my_covid_stat = 2 [ set color white ]
  if has_app = 1 [set shape "phone"]
  if my_health_status = 3 [set shape "x" set size 2]
  ]
end
...
to update-global-variables
  if count turtles > 0
    [ set %infected (count turtles with [ my_covid_stat = 1 ] / count turtles) * 100
      set dead_people (count turtles with [my_health_status = 3])
      ...
      set blocked_by_app (count turtles with [my_app_status = 1])
  ]
end
...
to get_older ;; turtle procedure for disease evolution in days
  if my_covid_stat = 1 [
    ...
    if time_since_infected > 12 [ifelse random-float 100 < 45 [ ; 30 to 60% of person never have severe covid
      set my_health_status 2
      set my_move 0]
      [set my_health_status 1]]
  ]
  if my_app_status = 1 [
    set time_since_blocked_by_app time_since_blocked_by_app + 1 ]
...
to save_list_app-turtles
    ask turtles [
      ...
      ask prey [
        if has_app = 1 [
        set my_list_contact lput temp_name my_list_contact ; save the prey name on its phone app
      ]]
      set my_list_contact lput ([who] of prey) my_list_contact
  ]]
end

to alert_list_app-turtles
  if my_health_status >= 1 [ ; when you fill sick you alert
    if my_health_status != 3 [
    if has_app = 1 [
      if length my_list_contact >= 2 [
      foreach my_list_contact [ x -> ask turtle x [
          if my_health_status != 3 [
          set my_move 0
          set time_since_blocked_by_app 0
          set my_app_status 1
          ]]]
         ]
       ]
...
to do_you_deblock
  if my_app_status = 1 [
    if time_since_blocked_by_app >= 5 [ ; time of incubation is passed
      if my_covid_stat != 1 [ ; subject is not infected
        if my_health_status != 3 [ ; subject is not dead
        set my_move 3
        set my_app_status 0 ]
    ]]
  ]
...
202
222
** EPIDEMY APP SIMULATOR **\n\nYour role is to take the right decisions at the right moment to limit the spread of the epidemy and earn the maximum health points.\n\nThe Epidemy app alert people who have been in contact with someone that is infected. And block them home until they have symptoms or not.
12
0.0
1
...
true
15
Rectangle -1 true true 105 90 210 270
Rectangle -1 true true 105 30 120 90
Rectangle -16777216 true false 120 105 195 165
Rectangle -16777216 true false 120 210 135 210
...
Rectangle -16777216 true false 135 240 180 255
